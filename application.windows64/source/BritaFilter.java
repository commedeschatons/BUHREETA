...
public class BritaFilter extends PApplet {
  ...
  for(int i = 0; i < fft.specSize(); i++)
    ...
    //line(i, Math.round(val), i+1, Math.round(val));
      ...
      String filstr = "";
       ...
       text(filstr +" Cutoff @ " + Math.round(fil.frequency()) + " Hz", mouseX*(PApplet.parseInt(mouseX<(GUIthresh*width))) + PApplet.parseInt(mouseX>(GUIthresh*width))*width*GUIthresh, mouseY  );
        
        fill(255);
        
       text("Instructions: Press H/L for\n HPF/LPF, move mouse to\nchange f_cutoff, [/] to skip,\n O to open a new track,\n space to play/pause", width*0.5f, 35);
...
public void keyPressed() /// NOT USING
{
  SineWave mySine;
  Osc osc1;
 ...
 // if (key == )
  ...
  switch(key) {
   ...
   if (pitch > 0) {
      osc1 = new Osc(pitch*pitchmu, 0.2f, "saw");
...
class Osc implements AudioSignal
{
     ...
     {  
         ...
         alph = 0.9f;  // Decay constant for the envelope
         out.addSignal(this);
  ...
  static public void main(String[] passedArgs) {
    String[] appletArgs = new String[] { "BritaFilter" };
    if (passedArgs != null) {
